...
public class VehiculeImpl extends Abst implements VehiculeInter {
	...
	public int add(Vehicule arg) {
		int status = 0;
		try {
			String sql = "insert into vehicule (matricule,nbr_place,idParking,idCarburant,idMarque,color) values (?,?,?,?,?,?)";
			...
			ps.setLong(5, arg.getMarque().getIdMarque());
	...
	public Vehicule edit(Vehicule arg) {
		String sql = "UPDATE vehicule SET  idParking = ? , dispo = ? WHERE matricule = ? ";
		Connection con = Abst.getConnection();
		try {
	...
	public List<Vehicule> getAll() {
		...
		try {
			...
			while(rs.next()) {
				...
				vehicule.setIdVehicule(rs.getString("matricule"));
				...
				vehicule.setMarque(H.marque.getById(rs.getLong("idMarque")));
